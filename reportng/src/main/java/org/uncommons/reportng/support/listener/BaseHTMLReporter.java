...
public class BaseHTMLReporter implements IReporter {
    ...
    public void generateReport(final List<XmlSuite> xmlSuites, final List<ISuite> suites, final String outputDirectory) {
        try {
            ...
            System.out.println("mustache==" + mustache);
            final File reportsFolder = new File(outputDirectory + "/html");

            reportsFolder.mkdir();
            if (reportsFolder.exists()) {
                mustache.execute(new OutputStreamWriter(new FileOutputStream(outputDirectory + "/" + REPORT_OUTPUT), "utf-8"), getScope(suites)).flush();
            }
        } catch (Exception e) {
    ...
    private Map<String, Object> getScope(final List<ISuite> suites) {
        ...
        for (ISuite suite : suites) {
            ...
            System.out.println("suite====>" + suite.getName() + "---->" + suite.getResults().size());

            final List<ISuiteResult> results = new ArrayList<>(suite.getResults().values());

            for (ISuiteResult result : results) {
                if (StringUtils.equals(result.getTestContext().getSuite().getName(), suite.getName())) {
                    System.out.println("====>" + result.getTestContext().getName());
                    suiteResults.add(new TestResults(result.getTestContext()));
                }
            }
            System.out.println("suite+++++>" + suite.getName() + suiteResults.size());
            if (suiteResults.size() > 0) {
